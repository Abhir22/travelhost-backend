...
export class RoomImageResponse {
  ...
  constructor(roomimage: RoomImage) {
    ...
    if ('createdAt' in roomimage && roomimage.createdAt) this.createdAt = moment(roomimage.createdAt).format('YYYY-MM-DD');
    if ('updatedAt' in roomimage && roomimage.updatedAt) this.updatedAt = moment(roomimage.updatedAt).format('YYYY-MM-DD');
  }
}
